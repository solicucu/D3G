...
def build_feat2d(cfg):
    pooling_counts = cfg.MODEL.D3G.FEAT2D.POOLING_COUNTS  # [15,8,8] anet, [15] charades
    num_clips = cfg.MODEL.D3G.NUM_CLIPS  # 64 anet, 16 charades
    hidden_size = cfg.MODEL.D3G.FEATPOOL.HIDDEN_SIZE  # 512
    if cfg.MODEL.D3G.FEAT2D.NAME == "conv":
        return SparseConv(pooling_counts, num_clips, hidden_size)
    elif cfg.MODEL.D3G.FEAT2D.NAME == "pool":
        return SparseMaxPool(pooling_counts, num_clips)
    else:
        raise NotImplementedError("No such feature 2d method as %s" % cfg.MODEL.D3G.FEAT2D.NAME)
